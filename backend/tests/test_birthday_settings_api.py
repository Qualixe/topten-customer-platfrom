"""GET/PUT /api/v1/settings/birthday and POST .../send-now (all protected,
require settings.manage — see app.controllers.birthday_settings)."""
...
async def test_send_now_is_a_noop_when_disabled(client: AsyncClient) -> None:
    response = await client.post("/api/v1/settings/birthday/send-now")
    assert response.status_code == 200
    data = response.json()["data"]
    assert data == {"total": 0, "sent": 0, "failed": 0, "skipped_already_sent": 0}
