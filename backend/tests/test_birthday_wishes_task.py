"""Exercises the Celery task wrapper's hour gate directly (never touches
Celery/a real broker) — see app.tasks.birthday_wishes. The service-level
send behavior itself is covered by test_birthday_wishes.py."""
...
from app.tasks.birthday_wishes import run_hourly_birthday_check
...
async def test_noop_when_hour_does_not_match(db_session: AsyncSession) -> None:
    ...
    with (
        ...
        await run_hourly_birthday_check(session_factory=TestSessionLocal)
