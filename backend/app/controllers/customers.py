...
async def _build_customer_filters(
    ...
    *,
    ...
    # A customer counts as "verified" here via either of the two sources
    # GET /customers/verified also reads: a campaign's profile form
    # (CampaignRecipient, not a stored Customer column) or the standalone,
    # tokenless Forms feature (Customer.form_verified_at) — must stay in
    # sync with list_verified_customers so this filter and that page always
    # agree on who counts as verified.
    if verified is not None:
        ...
        verified_condition = or_(verified_exists, Customer.form_verified_at.is_not(None))
        filters.append(verified_condition if verified else ~verified_condition)
...
def _verified_customers_common_filters(
    ...
    """Filters that apply identically to both verification sources (campaign
    / standalone form) — same semantics as the equivalent filters on
    `GET /customers` (see `_build_customer_filters`), so the two pages never
    disagree about what "matching the filters" means."""
    filters: list[ColumnElement] = []
    if search:
        pattern = f"%{search}%"
