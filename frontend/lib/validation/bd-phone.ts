...
// (Robi), 019 (Banglalink). Local format only (leading 0, 11 digits total)
// — the +880 shown next to the input is decorative, not part of the value.
...
export function validateBdPhone(value: string): string | null {
  ...
  if (!BD_MOBILE_PATTERN.test(trimmed)) return "Invalid prefix — use 013–019.";
