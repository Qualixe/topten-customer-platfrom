...
export function StepAudience({
  ...
  // Live count for advanced rules — the static six already have counts
  // fetched up front (see `counts`), so only fetch here once an advanced
  // rule's required inputs are filled in. Every setState call below runs
  // inside a .then() callback (never synchronously at the top of the
  // effect), per this codebase's react-hooks/set-state-in-effect fix
  // pattern — even the "reset to null" branches route through a resolved
  // promise instead of calling setState eagerly. SPECIFIC_CUSTOMERS is the
  // one exception: its count is exactly the number of picks already made in
  // the UI, so it never needs a round trip to the server.
  useEffect(() => {
    let cancelled = false;
            ...
            return (
              ...
              >
                ...
                {isSelected && option.ruleType === "NEVER_RECEIVED_TYPE" && (
                  <div className="flex flex-col gap-1.5 pl-12">
                    <Label htmlFor="never-received-type">Campaign type</Label>
                    <Select value={historyCampaignType} onValueChange={(value) => setHistoryCampaignType(value ?? "")}>
                      <SelectTrigger id="never-received-type" className="max-w-xs">
                ...
                {isSelected && option.ruleType === "RECEIVED_TYPE_BEFORE_DATE" && (
                  <div className="flex flex-wrap gap-4 pl-12">
                    <div className="flex flex-col gap-1.5">
                      <Label htmlFor="received-type">Campaign type</Label>
                      <Select value={historyCampaignType} onValueChange={(value) => setHistoryCampaignType(value ?? "")}>
                        <SelectTrigger id="received-type" className="w-48">
                          <SelectValue placeholder="Select a type" />
                        </SelectTrigger>
                        <SelectContent>
                          {CAMPAIGN_TYPE_OPTIONS.map(([value, label]) => (
