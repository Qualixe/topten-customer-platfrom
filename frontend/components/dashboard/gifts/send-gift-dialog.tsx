      ...
      <DialogContent className="flex flex-col gap-4 sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            can schedule and send from the list below.
          </DialogDescription>
        </DialogHeader>

        {/* Keyed on open so a fresh instance mounts (clean search/selection
            state) each time the dialog is reopened. */}
        <SendGiftForm key={open ? "open" : "closed"} onClose={() => setOpen(false)} />
      </DialogContent>
    </Dialog>
  );
}

function SendGiftForm({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    listGiftCatalog({ pageSize: 100 })
      ...
      .catch(() => {
        // The select just stays empty — the submit button requires a pick.
      });
  }, []);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <FormField htmlFor="send-gift-item" label="Gift">
        <Select value={catalogItemId} onValueChange={(value) => setCatalogItemId(value ?? "")}>
          <SelectTrigger id="send-gift-item">
            <SelectValue>
              {() => catalog.find((item) => item.id === catalogItemId)?.name ?? "Select a gift"}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
            {catalog.map((item) => (
