...
function FailedNotificationsSkeleton() {
  return (
    <Card>
      <CardHeader>
...
function NotificationsWorkspaceSkeleton() {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex gap-1 rounded-lg bg-muted p-1 w-fit">
          {["All", "SMS", "Email", "WhatsApp"].map((tab) => (
            <Skeleton key={tab} className="h-6 w-16 rounded-md" />
          ))}
        </div>
        {/* Toolbar */}
...
export default function NotificationsLoading() {
  return (
    <div className="flex flex-col gap-6">
      ...
      <FailedNotificationsSkeleton />
      <NotificationsWorkspaceSkeleton />
    </div>
  );
}
